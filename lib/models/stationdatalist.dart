class Destination {
  String imageUrl; //รูปสถานที่
  String locationName; //ชื่อสถานที่
  String locationaddress; //ที่อยู่สถานที่
  String description; //คำอธิบายสถานที่โดยย่อ
  String howtotravel; //วิธีเดินทางไป

  Destination({
    required this.imageUrl,
    required this.locationName,
    required this.locationaddress,
    required this.description,
    required this.howtotravel,
  });
}

Map<String, String> connectPic = {
  'Taxi บริการที่หน้าสถานี': 'assets/images/taxi.png',
  'สายสีนำ้เงิน สถานีบางซื่อ': 'assets/images/mrtblueline.png',
  'สายสีนำ้เงิน สถานีเพชรบุรี': 'assets/images/mrtblueline.png',
  'สายสีนำ้เงิน สถานีบางขุนนนท์': 'assets/images/mrtblueline.png',
  'สายสีนำ้เงิน สถานีหัวลำโพง': 'assets/images/mrtblueline.png',
  'รถไฟฟ้าบีทีเอส สถานีพญาไท': 'assets/images/bts.png',
  'Airport link สถานีพญาไท': 'assets/images/Airlink.jpg',
  'Airport link สถานีมักกะสัน': 'assets/images/Airlink.jpg',
  'Airport link สถานีรามคำแหง': 'assets/images/Airlink.jpg',
  'Airport link สถานีหัวหมาก': 'assets/images/Airlink.jpg',
  'Airport link สถานีบ้านทับช้าง': 'assets/images/Airlink.jpg',
  'Airport link สถานีลาดกระบัง': 'assets/images/Airlink.jpg',
  'สายสีแดงเข้ม สถานีกรุงเทพอภิวัฒน์': 'assets/images/darkredline.png',
  'สายสีแดงเข้ม สถานีดอนเมือง': 'assets/images/darkredline.png',
  'สายสีแดงเข้ม สถานีรังสิต': 'assets/images/darkredline.png',
  'สายสีแดงอ่อน สถานีกรุงเทพอภิวัฒน์': 'assets/images/lightredline.png',
  'สายสีแดงอ่อน สถานีบางบำหรุ': 'assets/images/lightredline.png',
  'สายสีแดงอ่อน สถานีตลิ่งชัน': 'assets/images/lightredline.png',
  'รถสี่ล้อแดง ผู้โดยสารเลือกปลายทางได้': 'assets/images/bus.png',
  'รถโดยสารประจำทาง ขสมก.': 'assets/images/bus.png',
  'shuttle bus ไปสถานีหัวลำโพง': 'assets/images/bus.png',
  'shuttle bus ไปสถานีกลางอภิวัฒน์': 'assets/images/bus.png',
  'ท่าอากาศยานดอนเมือง': 'assets/images/local-airport.png',
  'ท่าอากาศยานสุวรรณภูมิ': 'assets/images/local-airport.png',
  'รถสองแถวรอบเมืองฉะเชิงเทรา': 'assets/images/bus.png',
  'รถสองแถวรอบเมืองพัทยา': 'assets/images/bus.png',
  
  
};

Map<String, String> comfortPic = {
  'จำหน่ายตั๋วล่วงหน้า': 'assets/images/ticket.png',
  'จำหน่ายตั๋วประจำวัน': 'assets/images/ticket.png',
  'ห้องนำ้สะอาด': 'assets/images/toliet.png',
  'ห้องนำ้ผู้พิการ': 'assets/images/handicaptoliet.png',
  'ร้านอาหาร': 'assets/images/restaurant.png',
  'ร้านค้าต่างๆ': 'assets/images/shops.png',
};

class StationList {
  String name;
  String postition;
  List<Destination> places;
  List<String> connects;
  List<String> comforts;
  List<String> lines;
  StationList({
    required this.name,
    required this.postition,
    required this.places,
    required this.connects,
    required this.comforts,
    required this.lines,
  });
}

List<StationList> stationLists = [
  StationList(
      name: 'กบินทร์บุรี',
      postition: ' ตำบลกบินทร์ อำเภอกบินทร์บุรี จังหวัดปราจีนบุรี',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กระสัง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กรุงเทพ',
      postition: ' ถนนรองเมือง แขวงรองเมือง เขตปทุมวัน กรุงเทพ',
      places: [],
      connects: [
        'สายสีนำ้เงิน สถานีหัวลำโพง',
        'รถโดยสารประจำทาง ขสมก.',
        'shuttle bus ไปสถานีกลางอภิวัฒน์'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'กรุงเทพอภิวัฒน์',
      postition: ' ถนนเทอดดำริ แขวงจตุจักร เขตจตุจักร กรุงเทพ',
      places: [],
      connects: [
        'สายสีนำ้เงิน สถานีบางซื่อ',
        'สายสีแดงเข้ม สถานีกรุงเทพอภิวัฒน์',
        'สายสีแดงอ่อน สถานีกรุงเทพอภิวัฒน์',
        'Taxi บริการที่หน้าสถานี',
        'รถโดยสารประจำทาง ขสมก.',
        'shuttle bus ไปสถานีหัวลำโพง'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'กันตัง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กันทรารมย์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กาญจนบุรี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'การเคหะ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กุดจิก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'กุมภวาปี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ขอนแก่น',
      postition: ' ตำบลในเมือง อำเภอเมือง จังหวัดขอนแก่น',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ขุนตาน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองจันดี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองจาก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองตัน',
      postition: ' แขวงบางกะปี เขตห้วยขวาง กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองต้นไทร',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองนกเล็ก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองบางพระ',
      postition: ' ตำบลบางเตย อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองหลวงแพ่ง',
      postition: ' แขวงขุมทอง เขตลาดกระบัง กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองอุดมชลจร',
      postition: '  ตำบลอุดมชลจร อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คลองแขวงกลั่น',
      postition: ' ตำบลบางเตย อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คอกควาย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'คีรีรัฐนิคม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'จอมทอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'จักราช',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'จัตุรัส',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'จันเสน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ฉวาง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชลบุรี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชะอวด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชะอำ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางคลองสิบเก้า',
      postition: ' ตำบลโยธะกา อำเภอบางนำ้เปรี้ยว จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางฉะเชิงเทรา',
      postition: 'ตำบลหน้าเมือง อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [
         Destination(
        imageUrl: 'assets/images/sotorn.jpg',
        locationName: 'วัดโสธรวรารามวรวิหาร',
        locationaddress: '	ถนนเทพคุณากร ตำบลหน้าเมือง อำเภอเมืองฉะเชิงเทรา จังหวัดฉะเชิงเทรา',
        description:
            ' วัดหลวงพ่อโสธร เป็นวัดเก่าแก่ที่สร้างมาตั้งแต่สมัยกรุงศรีอยุธยาตอนปลาย ภายในประดิษฐาน หลวงพ่อพระพุทธโสธร หรือที่เรียกกันว่า หลวงพ่อโสธร พระรูปปางสมาธิ ลงรักปิดทองสมัยแบบล้านช้าง เป็นสิ่งศักดิ์สิทธิ์คู่บ้านคู่เมืองฉะเชิงเทรา',
        howtotravel:
            'จากสถานีฉะเชิงเทรา มีรถสองแถวสีเหลืองมารอรับในสถานีรถไฟเพื่อเดินทางต่อไปยังวัดโสธรวรารามวรวิหารได้เลย'),
            Destination(
        imageUrl: 'assets/images/talatbanmai100.jpg',
        locationName: 'ตลาดบ้านใหม่ 100 ปี',
        locationaddress: '	ถนนศุภกิจ ตำบลหน้าเมือง อำเภอเมือง จังหวัดฉะเชิงเทรา',
        description:
            ' เป็นตลาดริมแม่น้ำบางปะกงที่มีมาตั้งแต่ในสมัยรัชกาลที่ 5 ที่นี่เป็นชุมชนริมน้ำที่เคยมีการค้าขายกันอย่างคึกคักในสมัยเก่า จึงเป็นเหมือนพิพิธภัณฑ์มีชีวิตริมแม่น้ำบางปะกงที่ยังคงความดั้งเดิม คลาสสิกไว้ มีความวินเทจมากๆ',
        howtotravel:
            'จากสถานีฉะเชิงเทรา ใช้รถสองแถวรับจ้างที่จอดหน้าสถานีรถไฟเพื่อเดินทางต่อไปยังตลาดบ้านใหม่ 100 ปีได้เลย'),
      ],
      connects: [
        'รถสองแถวรอบเมืองฉะเชิงเทรา'
      ],
      comforts: [
    'จำหน่ายตั๋วล่วงหน้า',
    'จำหน่ายตั๋วประจำวัน',
    'ห้องนำ้สะอาด',
    'ห้องนำ้ผู้พิการ',
    'ร้านอาหาร',
    'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ชุมทางตลิ่งชัน',
      postition: ' แขวงฉิมพลี เขตตลิ่งชัน กรุงเทพฯ',
      places: [],
      connects: [
        'สายสีแดงอ่อน สถานีตลิ่งชัน'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'ชุมทางถนนจิระ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางทุ่งสง',
      postition: ' ตำบลปากแพรก อำเภอทุ่งสง จ.นครศรีธรรมราช',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'ชุมทางบัวใหญ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางบางซื่อ',
      postition: '',
      places: [],
      connects: [
        'สายสีนำ้เงิน สถานีบางซื่อ',
        'สายสีแดงเข้ม สถานีกรุงเทพอภิวัฒน์',
        'สายสีแดงอ่อน สถานีกรุงเทพอภิวัฒน์',
        'Taxi บริการที่หน้าสถานี',
        'รถโดยสารประจำทาง ขสมก.',
        'shuttle bus ไปสถานีหัวลำโพง'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางบ้านดารา',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางบ้านภาชี',
      postition: ' ตำบลภาชี อำเภอภาชี จ.พระนครศรีอยุธยา',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ชุมทางศรีราชา',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางหนองปลาดุก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางหาดใหญ่',
      postition: ' ตำบลหาดใหญ่ อำเภอหาดใหญ่ จังหวัดสงขลา',
      places: [
        Destination(
        imageUrl: 'assets/images/cueachang.jpg',
        locationName: 'วัดฉือฉาง',
        locationaddress: '	ถนนศุภสารรังสรรค์ ตำบลหาดใหญ่ อำเภอหาดใหญ่ จังหวัดสงขลา',
        description:
            ' วัดฉือฉาง เป็นวัดจีน อยู่ใจกลางเมืองใกล้ย่านการค้าเมืองหาดใหญ่ เป็นวัดที่ก่อสร้างแบบจีน เป็นอาคาร 4 ชั้น เค้าว่าออกแบบโดยใช้ศิลปะทั้งไทย จีน ทิเบต สามารถเดินชมความสวยงามของพระพุทธรูป สถาปัตยกรรมที่แปลกตาได้รอบวัด ซึ่งจะมีความแตกต่างจากวัดของไทยเราอย่างสิ้นเชิง',
        howtotravel:
            'จากสถานีชุมทางหาดใหญ่ เดินตามถนนศุภสารรังสรรค์ ไม่กี่ก้าวก็ถึงแล้ว'),
        Destination(
        imageUrl: 'assets/images/hatyaicablecar.jpg',
        locationName: 'กระเช้าลอยฟ้านครหาดใหญ่',
        locationaddress: '	สวนสาธารณะเทศบาลนครหาดใหญ่',
        description:
            '“กระเช้าลอยฟ้า”  แห่งแรกในประเทศไทย  ที่พร้อมด้วยความสะดวก ปลอดภัย  มีความทันสมัย  และรับรองด้วยมาตรฐานยุโรป  ซึ่งนอกจากนักท่องเที่ยวจะได้รับความสะดวกในการเดินทางจากพระพุทธมงคลมหาราชไปยังท้าวมหาพรหม  ภายในสวนสาธารณะเทศบาลนครหาดใหญ่แล้ว  ท่านยังเต็มตาในการเหิรฟ้าชมทัศนียภาพของเมืองหาดใหญ่  ประสบการณ์ที่น่าลองสัมผัส',
        howtotravel:
            'จากสถานีชุมทางหาดใหญ่ ใช้รถรับจ้างไปส่งที่สวนสาธารณะเทศบาลนครหาดใหญ่ได้'),
            Destination(
        imageUrl: 'assets/images/Songkhla_Lake.jpg',
        locationName: 'ทะเลสาบสงขลา',
        locationaddress: '	เทศบาลนครสงขลา',
        description:
            'เป็นทะเลสาบแห่งเดียวในประเทศไทย อยู่ติดกับจังหวัดพัทลุงและจังหวัดสงขลา และอ่าวไทย เป็นทะเลสาบที่มีลักษณะของน้ำในทะเลสาบถึง 3 น้ำ คือ น้ำจืด, น้ำกร่อย และน้ำเค็ม มีแม่น้ำและคลองหลายสายที่ไหลเข้าสู่ทะเลสาบแห่งนี้ ทำให้น้ำมีการเปลี่ยนแปลงเสมอตามฤดูกาล',
        howtotravel:
            'จากสถานีชุมทางหาดใหญ่ ใช้รถสาธารณะไปเทศบาลเมืองสงขลาได้'),
      ],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ชุมทางเขาชีจรรย์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางเขาชุมทอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมทางแก่งคอย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ชุมพร',
      postition: ' ตำบลท่าตะเภา อำเภอเมือง จังหวัดชุมพร',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ชุมแสง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ช่องแค',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ซอยวัดลานบุญ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ญาณสังวราราม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ดอนสีนนท์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ดอนเมือง',
      postition: ' แขวงสนามบิน เขตดอนเมือง กรุงเทพฯ',
      places: [
         Destination(
        imageUrl: 'assets/images/donmueng.jpg',
        locationName: 'ท่าอากาศยานดอนเมือง',
        locationaddress: '	ถนนวิภาวดีรังสิต แขวงสนามบิน เขตดอนเมือง กรุงเทพฯ',
        description:
            'เป็นสนามบินสำหรับเที่ยวบินที่ไม่ไกลมาก เช่น ภายในประเทศ ประเทศเพื่อนบ้าน ผู้โดยสามารถสามารถเดินทางจากสนามบินนี้ได้ไม่ยาก',
        howtotravel:
            'จากอาคารสถานีรถไฟ สามารถใช้สะพานลอยคนข้ามเดินเข้ามาที่สนามบินโดยตรง'),
      ],
      connects: [
        'สายสีแดงเข้ม สถานีดอนเมือง',
        'ท่าอากาศยานดอนเมือง'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ'
      ],
      lines: []),
  StationList(
      name: 'ตรอน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ตรัง',
      postition: ' ตำบลทับเที่ยง อำเภอเมือง จังหวัดตรัง',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ตลาดพลู',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ตลาดลำนารายณ์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ตะพานหิน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ตันหยงมัส',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ทับกฤช',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ทับสะแก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ทุ่งสีทอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่ากิเลน',
      postition: ' ตำบลสิงห์ อำเภอไทรโยค จ.กาญจนบุรี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
      ],
      lines: []),
  StationList(
      name: 'ท่าฉลอม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่าชนะ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่าสัก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่าฬ่อ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่าเกษม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ท่าเรือ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ธนบุรี',
      postition: ' แขวงศิริราช เขตบางกอกน้อย กรุงเทพ',
      places: [
         Destination(
        imageUrl: 'assets/images/siriraj_pimukstan.jpg',
        locationName: 'พิพิธภัณฑ์ศิริราชพิมุขสถาน',
        locationaddress: '	แขวงศิริราช เขตบางกอกน้อย กรุงเทพมหานคร',
        description:
            'มีความสำคัญในฐานะเคยเป็นสถานีต้นทางแห่งแรกของรถไฟสายใต้ (สถานีธนบุรี) ตัวสถานีรถไฟได้รับการออกแบบโดยหม่อมเจ้าโวฒยากร วรวรรณ ซึ่งสร้างแล้วเสร็จในปี พ.ศ. 2493 ยกเลิการใช้งานรถไฟเมื่อ 3 ตุลาคม 2546 เนื่องจากมอบพื้นที่ให้กับโรงพยาบาลศิริราช',
        howtotravel:
            'จากสถานีรถไฟธนบุรี เดินตามถนนพระนเรศวร ทางเดียวกับไปทางโรงพยาบาลศิริราชปิยมหาราชการุณย์'),
        Destination(
        imageUrl: 'assets/images/siriraj.jpg',
        locationName: 'โรงพยาบาลศิริราช',
        locationaddress: '	ถนนวังหลัง แขวงศิริราช เขตบางกอกน้อย กรุงเทพฯ',
        description:
            'โรงพยาบาลศิริราชเคยเป็นที่ประทับรักษาพระประชวรของพระบาทสมเด็จพระบรมชนกาธิเบศร มหาภูมิพลอดุลยเดชมหาราช บรมนาถบพิตร และ สมเด็จพระนางเจ้าสิริกิติ์ พระบรมราชินีนาถ พระบรมราชชนนีพันปีหลวง ตลอดจนพระบรมวงศานุวงศ์หลายพระองค์และบุคคลสำคัญ',
        howtotravel:
            'จากสถานีรถไฟธนบุรี เดินตามถนนพระนเรศวร ผ่านวัดอมรินทราราม ก็จะถึงโรงพยาบาลศิริราช'),
      ],
      connects: [
        'สายสีนำ้เงิน สถานีบางขุนนนท์'
      ],
      comforts: [
    'จำหน่ายตั๋วล่วงหน้า',
    'จำหน่ายตั๋วประจำวัน',
    'ห้องนำ้สะอาด',
    'ห้องนำ้ผู้พิการ'
      ],
      lines: []),
  StationList(
      name: 'นครปฐม',
      postition: ' ต.พระปฐมเจดีย์ อ.เมืองนครปฐม จ.นครปฐม',
      places: [
        Destination(
        imageUrl: 'assets/images/sanamchandra.jpg',
        locationName: 'พระราชวังสนามจันทร์',
        locationaddress: '	ตำบลพระปฐมเจดีย์ อำเภอเมือง จังหวัดนครปฐม',
        description:
            'พระราชวังสนามจันทร์ ห่างจากพระปฐมเจดีย์ ประมาณ 2 กิโลเมตร สร้างขึ้นโดยพระบาทสมเด็จพระมงกุฎเกล้าเจ้าอยู่หัว รัชกาลที่ 6 หลังจากพระองค์สวรรคต พระราชวังสนามจันทร์ใช้เป็นที่ทำการของส่วนราชการต่าง ๆ ของจังหวัดนครปฐม รวมทั้งเป็นวิทยาเขตหนึ่งของ มหาวิทยาลัยศิลปากร',
        howtotravel:
            ' ที่สถานีนครปฐม ให้ต่อรถรถจักรยานยนต์รับจ้างไปพระราชวังสนามจันทร์ สามารถเข้าพระราชวังสนามจันทร์ผ่านทางด้านมหาวิทยาลัยศิลปากรได้'),
        Destination(
        imageUrl: 'assets/images/pathomchedi.png',
        locationName: 'วัดพระปฐมเจดีย์',
        locationaddress: '	ตำบลพระปฐมเจดีย์ อำเภอเมือง จังหวัดนครปฐม',
        description:
            'วัดพระปฐมเจดีย์ เป็นพระอารามหลวงชั้นเอกชนิดราชวรมหาวิหาร มีจุดเด่นที่สำคัญคือ พระปฐมเจดีย์ หรือ พระธมเจดีย์ ซึ่งเป็นเจดีย์ที่ใหญ่และสูงที่สุดแห่งหนึ่งในประเทศไทย (120.45 เมตร) องค์พระปฐมเจดีย์ปัจจุบันนี้ เป็นพระเจดีย์ทรงลังกาแบบสุโขทัย สร้างขึ้นในสมัยพระบาทสมเด็จพระจอมเกล้าเจ้าอยู่หัว',
        howtotravel:
            ' จากสถานีนครปฐม เดินไปไม่กี่ก้าวประมาณ 400 เมตร ก็ถึงแล้ว'),
      ],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'นครราชสีมา',
      postition: ' ตำบลในเมือง อำเภอเมือง จังหวัดนครราชสีมา',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'นครลำปาง',
      postition: ' ตำบลสบตุ๋ย อำเภอเมือง จังหวัดลำปาง',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'นครสวรรค์',
      postition: ' ตำบลหนองปลิง อำเภอเมือง จังหวัดนครสวรรค์',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'นาบอน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'นาสาร',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'น้ำตก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'น้ำพอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางกระทุ่ม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางกระบูน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางกระเจ้า',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางน้ำจืด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางน้ำเปรี้ยว',
      postition: ' ตำบลบางขนาก อำเภอบางนำ้เปรี้ยว จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางบอน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางบำหรุ',
      postition: ' แขวงบางพลัด เขตบางพลัด กรุงเทพฯ',
      places: [],
      connects: [
        'สายสีแดงอ่อน สถานีบางบำหรุ'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
    'จำหน่ายตั๋วประจำวัน',
    'ห้องนำ้สะอาด',
    'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'บางปะอิน',
      postition: ' ตำบลบางเลน อำเภอบางปะอิน จ.พระนครศรีอยุธยา',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'บางพระ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางมูลนาก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางระมาด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางละมุง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางสะพานน้อย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางสะพานใหญ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางสีคต',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางเตย',
      postition: ' ตำบลบางเตย อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บางโทรัด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บำเหน็จณรงค์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บุรีรัมย์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านกรูด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านกลับ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านกาหลง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านขอม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านฉิมพลี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านชีผ้าขาว',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านดงบัง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านตาคลี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านทับช้าง',
      postition: ' แขวงประเวศ เขตประเวศ กรุงเทพฯ',
      places: [],
      connects: [
        'Airport link สถานีบ้านทับช้าง'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านนาขวาง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านนาโคก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านบ่อ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านปากพลี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านปิน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านป่าหวาย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านพรหมแสง',
      postition: ' ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านพลูตาหลวง',
      postition: ' ตำบลพลูตาหลวง อำเภอสัตหีบ จังหวัดชลบุรี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'บ้านสร้าง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านส้อง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านหมอ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านหมี่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านห้วยขวาง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านเหลื่อม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านแก้ง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านแหลม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านโป่ง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านใหม่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'บ้านไผ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ประจวบคีรีขันธ์',
      postition: ' ต.ประจวบคีรีขันธ์ อ.เมือง จ.ประจวบคีรีขันธ์',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ประจันตคาม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปราจีนบุรี',
      postition: ' ตำบลหน้าเมือง อำเภอเมือง จังหวัดปราจีนบุรี',
      places: [
        Destination(
        imageUrl: 'assets/images/abhaibhubejhr.jpg',
        locationName: 'โรงพยาบาลเจ้าพระยาอภัยภูเบศร',
        locationaddress: '	ถนนปราจีนอนุสรณ์ ตำบลท่างาม อำเภอเมือง จังหวัดปราจีนบุรี',
        description:
            'โรงพยาบาลเจ้าพระยาอภัยภูเบศร เป็นโรงพยาบาลศูนย์ประจำจังหวัดปราจีนบุรี และเป็นโรงพยาบาลนำร่องเรื่องการแพทย์แผนไทย ใช้สมุนไพรบำบัดรักษาโรค มีการนวด อบ ประคบ และฝังเข็ม แปรรูปสมุนไพรไทยเป็นเวชภัณฑ์ และเครื่องสำอาง ตึกเจ้าพระยาอภัยภูเบศร มีลักษณะสถาปัตยกรรมเป็นสถาปัตยกรรมบาโรก แสดงออกอย่างไม่สมดุล',
        howtotravel:
            ' จากสถานีปราจีนบุรี ให้ต่อรถตุ๊กตุ๊ก ให้บอกไปโรงพยาบาลเจ้าพระยาอภัยภูเบศร'),
        Destination(
        imageUrl: 'assets/images/chakarapong.jpg',
        locationName: 'พิพิธภัณฑ์ทหารจักรพงษ์',
        locationaddress: '	กองบัญชาการมณฑลทหารบกที่ 12 ตำบลดงพระราม อำเภอเมือง จังหวัดปราจีนบุรี',
        description:
            'พิพิธภัณฑ์ทหารค่ายจักรพงษ์เป็นตึกเหลืองสไตล์ยุโรปตั้งอยู่บริเวณมณฑลทหารบกที่ 12 ค่ายทหารแห่งแรกในประเทศไทย ต่อมากรมศิลปากรได้ขึ้นทะเบียนเป็นตึกอนุรักษ์และเริ่มทำเป็นพิพิธภัณฑ์ เพื่ออนุรักษ์เป็นสถานที่เยี่ยมชมประวัติศาสตร์แก่ลูกหลาน  มีห้องทำงานและมุมพักผ่อนอิริยาบถ ส่วนวัตถุจัดแสดงตามห้องต่าง ๆ ในพิพิธภัณฑ์นี้ส่วนใหญ่เป็นเครื่องใช้ในการทหาร ถ้วยโถชามกระเบื้องแบบยุโรป สิ่งของทุกชิ้นที่จัดแสดงจะมีตราของกองทัพบก',
        howtotravel:
            ' จากสถานีปราจีนบุรี เดินเข้ากองบัญชาการมณฑลทหารบกที่ 12 ก็ถึงแล้ว'),
      ],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'ปราณบุรี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปะทิว',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปัตตานี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปากช่อง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปากท่อ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปากน้ำโพ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ปาดังเบซาร์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ผาเสด็จ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พญาไท',
      postition: ' แขวงถนนพญาไท เขตราชเทวี กรุงเทพฯ',
      places: [],
      connects: [
        'รถไฟฟ้าบีทีเอส สถานีพญาไท',
        'Airport link สถานีพญาไท'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'พรมแดน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พรหมพิราม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พระจอมเกล้า',
      postition: ' แขวงลาดกระบัง เขตลาดกระบัง กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พระปรง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พัทยา',
      postition: ' ตำบลนาเกลือ อำเภอบางละมุง จังหวัดชลบุรี',
      places: [
        Destination(
        imageUrl: 'assets/images/Naklua.jpg',
        locationName: 'หาดพัทยา',
        locationaddress: '	เทศบาลเมืองพัทยา',
        description:
            'เป็นชายหาดที่มีคนพลุกพล่านอยู่ตลอดเวลา แถมยังอยู่ใกล้สถานบันเทิง แหล่งท่องเที่ยวสำคัญๆ หลายจุด เช่น Walking Stress, Central Festival Pattaya Beach, Terminal 21 และที่พักใกล้ชายหาด เดินทางง่าย สะดวก มีรถโดยสารสัญจรไปมาอยู่ตลอด',
        howtotravel:
            'จากสถานีพัทยา สามารถใช้รถสองแถวเข้าตัวเมืองพัทยา ก็สามารถไปหาดพัทยาได้'),
            Destination(
        imageUrl: 'assets/images/Jomtien_Beach.jpg',
        locationName: 'หาดจอมเทียน',
        locationaddress: '	เทศบาลเมืองพัทยา อยู่ห่างจากพัทยาใต้ 4 กิโลเมตร',
        description:
            'หาดจอมเทียน เป็นหาดทรายสวยงามทอดตัวเป็นแนวยาว อยู่ห่างจากพัทยาใต้ 4 กิโลเมตร ตามถนนเลียบชายหาดหรือแยกขวาจากถนนสุขุมวิท ตรงกิโลเมตรที่ 150.5 เข้าไปอีก 2 กิโลเมตร หาดแห่งนี้ได้รับความนิยมจากนักท่องเที่ยว จึงทำให้มีธุรกิจเกิดขึ้นมากมาย โดยเฉพาะ ธุรกิจ ที่พัก และ โรงแรมเป็นหาดยอดนิยม ของเมืองพัทยา',
        howtotravel:
            'จากสถานีพัทยา สามารถใช้รถสองแถวเข้าตัวเมืองพัทยา ไปพัทยาใต้ ก็สามารถไปหาดจอมเทียนได้'),
      ],
      connects: [
        'รถสองแถวรอบเมืองพัทยา'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'พัทยาใต้',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พัทลุง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พานทอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พิจิตร',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พิชัย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'พิษณุโลก',
      postition: ' ตำบลในเมือง อำเภอเมือง จังหวัดพิษณุโลก',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'มวกเหล็ก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'มหาชัย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'มักกะสัน',
      postition: ' แขวงมักกะสัน เขตราชเทวี กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'มาบกะเบา',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'มาบอำมฤต',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ยะลา',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'รพ.รามาธิบดี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'รังสิต',
      postition: ' ตำบลประชาธิปัตย์ อำเภอธัญบุรี จังหวัดปทุมธานี',
      places: [],
      connects: [
        'สายสีแดงเข้ม สถานีรังสิต'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'รางสะแก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'รางโพธิ์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ราชบุรี',
      postition: ' ตำบลหน้าเมือง อำเภอเมือง จังหวัดราชบุรี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ราชปรารภ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลพบุรี',
      postition: ' ตำบลท่าหิน อำเภอเมือง จังหวัดลพบุรี',
      places: [
        Destination(
        imageUrl: 'assets/images/Phra_Prang_Sam_Yod.jpg',
        locationName: 'พระปรางค์สามยอด',
        locationaddress: ' ตำบลท่าหิน อำเภอเมืองลพบุรี จังหวัดลพบุรี',
        description:
            'เป็นปราสาทศิลาแลงแบบขอมเรียงต่อกัน 3 องค์ เชื่อมต่อกันด้วยมุขกระสัน ภายในบริเวณนอกจากปราสาททั้ง 3 องค์นี้แล้ว ทางด้านทิศตะวันออกของปราสาทประธานมีการต่อเติมวิหารก่ออิฐถือปูนเชื่อมต่อกับปราสาทประธาน เพื่อประดิษฐานพระพุทธรูปในรัชสมัยสมเด็จพระนารายณ์มหาราช',
        howtotravel:
            'จากสถานีรถไฟลพบุรี เลี้ยวขวาเดินตามถนนหน้าศาลพระกาฬ เดินไม่กี่ก้าวก็ถึงแล้ว'),
            Destination(
        imageUrl: 'assets/images/Wat_Mahathat.jpg',
        locationName: 'วัดพระศรีรัตนมหาธาตุ',
        locationaddress: ' ตำบลท่าหิน อำเภอเมืองลพบุรี จังหวัดลพบุรี',
        description:
            ' วัดพระศรีรัตนมหาธาตุ เป็นวัดอารามใน ตั้งอยู่ตรงข้ามกับสถานีรถไฟลพบุรี สร้างในสมัยใดไม่ปรากฏหลักฐานแน่ชัด แต่มีการปรับปรุงซ่อมแซมหลายครั้งทั้งในสมัยสมเด็จพระนารายณ์มหาราช สมเด็จพระราเมศวร และสมเด็จพระมหาจักรพรรดิ องค์ประธานวัดพระศรีรัตนมหาธาตุยังปรากฏบนผลงานภาพจิตรกรรมฯ 8 จอมเจดีย์แห่งสยาม ณ วัดเบญจมบพิตร อีกด้วย',
        howtotravel:
            'จากสถานีรถไฟลพบุรี จะอยู่ตรงข้ามอาคารสถานีโดยตรง'),
            Destination(
        imageUrl: 'assets/images/Narai_Ratcha_Niwet.jpg',
        locationName: 'พระนารายณ์ราชนิเวศน์',
        locationaddress: ' ตำบลท่าหิน อำเภอเมืองลพบุรี จังหวัดลพบุรี',
        description:
            ' เป็นพระราชวังที่สมเด็จพระนารายณ์มหาราชโปรดให้สร้างขึ้นเมื่อปี พ.ศ. 2209 บนพื้นที่ 41 ไร่ เพื่อใช้สำหรับเป็นที่ประทับ ล่าสัตว์ ออกว่าราชการ และต้อนรับแขกเมือง ภายหลังการสวรรคตของสมเด็จพระนารายณ์มหาราช พระราชวังฯแห่งนี้จึงถูกทิ้งร้าง ต่อมาพระบาทสมเด็จพระจอมเกล้าเจ้าอยู่หัว ทรงโปรดเกล้าฯ ให้บูรณะซ่อมแซมขึ้นใหม่อีกครั้งเมื่อปี พ.ศ. 2399 เพื่อใช้สำหรับเป็นที่ประทับ และเป็นราชธานีชั้นใน ',
        howtotravel:
            'จากสถานีรถไฟลพบุรี เลี้ยวขวาเดินตามถนนพระยากำจัด เดินไม่กี่ก้าวก็ถึงแล้ว'),
      ],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ละแม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลาดกระบัง',
      postition: ' แขวงลาดกระบัง เขตลาดกระบัง กรุงเทพฯ',
      places: [],
      connects: [
        'Airport link สถานีลาดกระบัง',
        'ท่าอากาศยานสุวรรณภูมิ'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลาดใหญ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลำชี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลำปลายมาศ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ลำพูน',
      postition: ' ตำบลในเมือง อำเภอเมือง จังหวัดลำพูน',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ'
      ],
      lines: []),
  StationList(
      name: 'วงเวียนใหญ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'วังกรด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'วังก์พง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'วัฒนานคร',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'วัดสิงห์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'วัดไทร',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ศรีสะเกษ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ศาลาธรรมสพน์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ศาลายา',
      postition: ' ตำบลศาลายา อำเภอพุทธมณฑล จังหวัดนครปฐม',
      places: [
         Destination(
        imageUrl: 'assets/images/phutthamonton.jpg',
        locationName: 'พุทธมณฑล',
        locationaddress: ' ถนนพุทธมณฑล สาย 4 ตำบลศาลายา อำเภอพุทธมณฑล จังหวัดนครปฐม',
        description:
            'เป็นสถานที่สำคัญทางพุทธศาสนา  มีเนื้อที่ 2,500 ไร่ สร้างขึ้นเพื่อฉลองวาระกึ่งพุทธกาล เมื่อ พ.ศ. 2500 มีพระพุทธรูปปางลีลาประจำพุทธมณฑล เรียกว่า พระศรีศากยะทศพลญาณ ประธานพุทธมณฑลสุทรรศน์ พระพุทธรูปนี้มีความโดดเด่นทางผ้าจีวรที่พลิ้วเหมือนจริง พระพุทธรูปนี้สร้างเสร็จและฉลอง เมื่อ พ.ศ. 2525 คราวพระราชพิธีสมโภชกรุงรัตนโกสินทร์ 200 ปี',
        howtotravel:
            'จากสถานีรถไฟศาลายา สามารถใช้รถแท็กซี่ไปตามถนนพุทธมณฑลสาย 4 เพียงไม่ถึง 15 นาที ก็ถึงพุทธมณฑลแล้ว'),
      ],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ศาลาลำดวน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ศิลาอาสน์',
      postition: ' ตำบลท่าเสา อำเภอเมือง จังหวัดอุตรดิตถ์',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ศีขรภูมิ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สระบุรี',
      postition: ' ตำบลปากเพรียว อำเภอเมือง จังหวัดสระบุรี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'สระแก้ว',
      postition: ' ตำบลสระแก้ว อำเภอเมือง จังหวัดสระแก้ว',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'สวนนงนุช',
      postition: ' ตำบลนาจอมเทียน อำเภอสัตหีบ จังหวัดชลบุรี',
      places: [
        Destination(
        imageUrl: 'assets/images/nongnuch.jpg',
        locationName: 'สวนนงนุชพัทยา',
        locationaddress: '	หมู่ที่ 7 ตำบลนาจอมเทียน อำเภอสัตหีบ จังหวัดชลบุรี',
        description:
            ' ภายในพื้นที่สวนนงนุช มีเนื้อที่ประมาณ 1,700 ไร่ เก็บรวบรวมพันธุ์ไม้ในเขตร้อนมากกว่า 18,000 ชนิด และมีสวนไม้ดอกไม้ประดับนานาชนิด เช่น กล้วยไม้ เฟิน สับปะรดสี สวนไม้พุ่ม ไม้ดัด สวนหิน สวนฝรั่งเศสกระบองเพชร ปาล์มจากทั่วทุกมุมโลก และต้นไม้ยักษ์ รวมทั้งสวนสัตว์ สวนผีเสื้อ ไดโนเสาร์',
        howtotravel:
            'จากที่หยุดรถสวนนงนุช รถไฟจะส่งที่ด้านหน้าสวนโดยตรง'),
      ],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สวรรคโลก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สวี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สะพานถ้ำกระแซ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สะพานแควใหญ่',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สามเสน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สามแยก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สำโรงทาบ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สีคิ้ว',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สุขุมวิท 71',
      postition: '',
      places: [],
      connects: [
        'Airport link สถานีรามคำแหง'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'สุพรรณบุรี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สุราษฎร์ธานี',
      postition: ' ทล.4008 ตำบลท่าข้าม อำเภอพุนพิน จ.สุราษฎร์ธานี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'สุรินทร์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สุไหงโก,ลก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'สูงเนิน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองกวย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองคาย',
      postition: ' ตำบลมีชัย อำเภอเมือง จังหวัดหนองคาย',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'หนองตม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองบัวลาย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองวิวัฒน์',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองสัง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองโดน',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หนองโพ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หลังสวน',
      postition: ' ตำบลหลังสวน อำเภอหลังสวน จังหวัดชุมพร',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(
      name: 'หัวตะเข้',
      postition: ' แขวงลาดกระบัง เขตลาดกระบัง กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด'
      ],
      lines: [
        'สายตะวันออก'
      ]),
  StationList(
      name: 'หัวหมาก',
      postition: 'แขวงพัฒนาการ เขตสวนหลวง กรุงเทพฯ',
      places: [],
      connects: [
        'Airport link สถานีหัวหมาก'
      ],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด'
      ],
      lines: [
        'สายตะวันออก'
      ]),
  StationList(
      name: 'หัวหวาย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'หัวหิน',
      postition: ' ตำบลหัวหิน อำเภอหัวหิน จังหวัดประจวบคีรีขันธ์',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'ห้วยสามพาด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ห้วยเดื่อ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ห้วยแถลง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ห้วยโจด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'อยุธยา',
      postition: ' ตำบลกะมัง อำเภอเมือง จ.พระนครศรีอยุธยา',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'อรัญประเทศ',
      postition: ' ตำบลอรัญประเทศ อำเภออรัญประเทศ จังหวัดสระแก้ว',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'อุดรธานี',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'อุตรดิตถ์',
      postition: ' ตำบลท่าอิฐ อำเภอเมือง จังหวัดอุตรดิตถ์',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: [
        "สายเหนือ"
      ]),
  StationList(
      name: 'อุทุมพรพิสัย',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'อุบลราชธานี',
      postition: ' ตำบลคำน้ำแซบ อำเภอวารินชำราบ จ.อุบลราชธานี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'อุรุพงษ์',
      postition: ' แขวงทุ่งพญาไท เขตราชเทวี กรุงเทพฯ',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'อโศก',
      postition: ' แขวงมักกะสัน เขตราชเทวี กรุงเทพฯ',
      places: [],
      connects: [
        'Airport link สถานีมักกะสัน',
        'สายสีนำ้เงิน สถานีเพชรบุรี'
      ],
      comforts: [],
      lines: []),
  StationList(
      name: 'เกาะมหามงคล',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เขตเมือง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เขาทอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เขาพระบาท',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เขาสวนกวาง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เชียงราก',
      postition: ' ตำบลคลองหนึ่ง อำเภอคลองหลวง จังหวัดปทุมธานี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
      ],
      lines: []),
  StationList(name: 'เชียงใหม่', postition: ' ถนนเจริญเมือง ตำบลวัดเกต อำเภอเมือง จังหวัดเชียงใหม่', places: [
    Destination(
        imageUrl: 'assets/images/Tha-Phae-Gate.jpg',
        locationName: 'ประตูท่าแพ',
        locationaddress: 'ถนนท่าแพ ตำบลช้างคลาน อำเภอเมือง จังหวัดเชียงใหม่',
        description:
            'ประตูที่ตั้งอยู่ทางทิศตะวันออก และเป็น 1 ใน 5 ประตูเมืองชั้นในของเวียงเชียงใหม่อีกด้วย ซึ่งถือได้ว่าปัจจุบันนี้ ที่นี่เป็นแค่เพียงประตูเมืองเพียงแห่งเดียวที่มีบานประตูนั่นเอง',
        howtotravel:
            'จากสถานีเชียงใหม่ ให้ใช้บริการรถสี่ล้อแดงไปประตูท่าแพได้'),
    Destination(
        imageUrl: 'assets/images/zoo.jpg',
        locationName: 'สวนสัตว์เชียงใหม่',
        locationaddress: 'ถนนห้วยแก้ว ตำบลสุเทพ อำเภอเมือง จังหวัดเชียงใหม่',
        description:
            'มีสัตว์อยู่จำนวนมาก เช่น เม่น นกยูง เสือโคร่ง เสือขาว กวาง แรด ฮิปโปเตมัส ช้าง หมี อีเห็น ยีราฟ และยังมีส่วนจัดแสดงหมีแพนด้า ช่วงช่วง และ หลินฮุ่ย ทูตจากประเทศจีนอีกด้วย',
        howtotravel:
            'จากสถานีเชียงใหม่ ให้ใช้บริการรถสี่ล้อแดงไปสวนสัตว์โดยตรง'),
            Destination(
        imageUrl: 'assets/images/doisuthep.jpg',
        locationName: 'วัดพระธาตุดอยสุเทพ',
        locationaddress: ' ถนนศรีวิชัย ตำบลสุเทพ อำเภอเมือง จังหวัดเชียงใหม่',
        description:
            'ตั้งอยู่ในอุทยานแห่งชาติดอยสุเทพ-ปุย วัดมีความสูงจากระดับที่ราบเชียงใหม่ราว 689 เมตร และมีความสูงจากระดับน้ำทะเลปานกลาง 1,046 เมตร เป็นหนึ่งในวัดที่มีความสำคัญมากที่สุดของจังหวัดเชียงใหม่ นอกจากนี้ยังประกอบด้วยจุดชมทิวทัศน์ที่สามารถมองเห็นเมืองเชียงใหม่ได้ทั้งหมด',
        howtotravel:
            'จากสถานีเชียงใหม่ ให้ใช้บริการรถสี่ล้อแดงไปดอยสุเทพโดยตรง'),
  ], connects: [
    'รถสี่ล้อแดง ผู้โดยสารเลือกปลายทางได้',
    'Taxi บริการที่หน้าสถานี'
  ], comforts: [
    'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
  ], lines: [
    "สายเหนือ"
  ]),
  StationList(
      name: 'เด่นชัย',
      postition: ' ตำบลเด่นชัย อำเภอเด่นชัย จังหวัดแพร่',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: [
        "สายเหนือ"
      ]),
  StationList(
      name: 'เนินมะกอก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เปรง',
      postition: ' ตำบลคลองเปรง อำเภอเมือง จังหวัดฉะเชิงเทรา',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เพชรบุรี',
      postition: ' ตำบลคลองกระแชง อำเภอเมือง จังหวัดเพชรบุรี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
  StationList(
      name: 'เมืองคง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'เมืองพล',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'แก่งเสือเต้น',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'แม่กลอง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'แม่เมาะ',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โคกกรวด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โคกกะเทียม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โคกมะกอก',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โนนสะอาด',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โนนสูง',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โพธาราม',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'โยทะกา',
      postition: '',
      places: [],
      connects: [],
      comforts: [],
      lines: []),
  StationList(
      name: 'ไชยา',
      postition: ' ตำบลตลาดไชยา อำเภอไชยา จ.สุราษฎร์ธานี',
      places: [],
      connects: [],
      comforts: [
        'จำหน่ายตั๋วล่วงหน้า',
        'จำหน่ายตั๋วประจำวัน',
        'ห้องนำ้สะอาด',
        'ห้องนำ้ผู้พิการ',
        'ร้านอาหาร',
        'ร้านค้าต่างๆ'
      ],
      lines: []),
];
//0 north
//1 east
//2 north east
//3 south
//4 สายวงเวียนใหญ่ - มหาชัย
//5 บ้านแหลม - แม่กลอง
var line = [
  [
    'กรุงเทพ',
    'สามเสน',
    'ชุมทางบางซื่อ',
    'กรุงเทพอภิวัฒน์',
    'ดอนเมือง',
    'รังสิต',
    'เชียงราก',
    'บางปะอิน',
    'อยุธยา',
    'ชุมทางบ้านภาชี',
    'หนองวิวัฒน์',
    'ท่าเรือ',
    'บ้านหมอ',
    'หนองโดน',
    'บ้านกลับ',
    'บ้านป่าหวาย',
    'ลพบุรี',
    'โคกกะเทียม',
    'บ้านหมี่',
    'จันเสน',
    'ช่องแค',
    'บ้านตาคลี',
    'หัวหวาย',
    'หนองโพ',
    'เนินมะกอก',
    'เขาทอง',
    'นครสวรรค์',
    'ปากน้ำโพ',
    'ทับกฤช',
    'ชุมแสง',
    'บางมูลนาก',
    'ตะพานหิน',
    'วังกรด',
    'พิจิตร',
    'ท่าฬ่อ',
    'บางกระทุ่ม',
    'บ้านใหม่',
    'พิษณุโลก',
    'พรหมพิราม',
    'หนองตม',
    'พิชัย',
    'ชุมทางบ้านดารา',
    'สวรรคโลก',
    'ท่าสัก',
    'ตรอน',
    'อุตรดิตถ์',
    'ศิลาอาสน์',
    'เด่นชัย',
    'บ้านปิน',
    'แม่เมาะ',
    'นครลำปาง',
    'ขุนตาน',
    'ลำพูน',
    'เชียงใหม่'
  ],
  [
    'กรุงเทพ',
    'สามเสน',
    'ชุมทางบางซื่อ',
    'กรุงเทพอภิวัฒน์',
    'ดอนเมือง',
    'รังสิต',
    'บางปะอิน',
    'อยุธยา',
    'ชุมทางบ้านภาชี',
    'หนองกวย',
    'สระบุรี',
    'ชุมทางแก่งคอย',
    'แก่งเสือเต้น',
    'ตลาดลำนารายณ์',
    'บำเหน็จณรงค์',
    'จัตุรัส',
    'บ้านเหลื่อม',
    'มาบกะเบา',
    'ผาเสด็จ',
    'มวกเหล็ก',
    'ปากช่อง',
    'สีคิ้ว',
    'สูงเนิน',
    'กุดจิก',
    'โคกกรวด',
    'นครราชสีมา',
    'ชุมทางถนนจิระ',
    'โนนสูง',
    'เมืองคง',
    'ชุมทางบัวใหญ่',
    'หนองบัวลาย',
    'เมืองพล',
    'บ้านไผ่',
    'ขอนแก่น',
    'น้ำพอง',
    'เขาสวนกวาง',
    'โนนสะอาด',
    'กุมภวาปี',
    'ห้วยสามพาด',
    'อุดรธานี',
    'หนองคาย',
    'จักราช',
    'ห้วยแถลง',
    'ลำปลายมาศ',
    'บุรีรัมย์',
    'กระสัง',
    'ลำชี',
    'สุรินทร์',
    'ศีขรภูมิ',
    'สำโรงทาบ',
    'อุทุมพรพิสัย',
    'ศรีสะเกษ',
    'กันทรารมย์',
    'อุบลราชธานี'
  ],
  [
    'กรุงเทพ',
    'อุรุพงษ์',
    'พญาไท',
    'มักกะสัน',
    'ราชปรารภ',
    'อโศก',
    'คลองตัน',
    'สุขุมวิท 71',
    'หัวหมาก',
    'บ้านทับช้าง',
    'ซอยวัดลานบุญ',
    'ลาดกระบัง',
    'พระจอมเกล้า',
    'หัวตะเข้',
    'คลองหลวงแพ่ง',
    'คลองอุดมชลจร',
    'เปรง',
    'คลองแขวงกลั่น',
    'คลองบางพระ',
    'บางเตย',
    'ชุมทางฉะเชิงเทรา',
    'ดอนสีนนท์',
    'บางน้ำเปรี้ยว',
    'ชุมทางคลองสิบเก้า',
    'พานทอง',
    'โยทะกา',
    'บ้านสร้าง',
    'ชลบุรี',
    'บ้านปากพลี',
    'บางพระ',
    'ปราจีนบุรี',
    'เขาพระบาท',
    'โคกมะกอก',
    'ชุมทางศรีราชา',
    'ประจันตคาม',
    'บางละมุง',
    'บ้านดงบัง',
    'บ้านพรหมแสง',
    'พัทยา',
    'พัทยาใต้',
    'กบินทร์บุรี',
    'บ้านห้วยขวาง',
    'ญาณสังวราราม',
    'หนองสัง',
    'สวนนงนุช',
    'ชุมทางเขาชีจรรย์',
    'บ้านพลูตาหลวง',
    'พระปรง',
    'บ้านแก้ง',
    'ศาลาลำดวน',
    'สระแก้ว',
    'ท่าเกษม',
    'ห้วยโจด',
    'วัฒนานคร',
    'ห้วยเดื่อ',
    'อรัญประเทศ'
  ],
  [
    'กรุงเทพ',
    'สามเสน',
    'ชุมทางบางซื่อ',
    'กรุงเทพอภิวัฒน์',
    'รพ.รามาธิบดี',
    'ธนบุรี',
    'บางระมาด',
    'ชุมทางตลิ่งชัน',
    'บางบำหรุ',
    'บ้านฉิมพลี',
    'ศาลาธรรมสพน์',
    'ศาลายา',
    'นครปฐม',
    'ชุมทางหนองปลาดุก',
    'สุพรรณบุรี',
    'กาญจนบุรี',
    'สะพานแควใหญ่',
    'ท่ากิเลน',
    'สะพานถ้ำกระแซ',
    'เกาะมหามงคล',
    'น้ำตก',
    'บ้านโป่ง',
    'โพธาราม',
    'ราชบุรี',
    'ปากท่อ',
    'เพชรบุรี',
    'ชะอำ',
    'หัวหิน',
    'วังก์พง',
    'ปราณบุรี',
    'ประจวบคีรีขันธ์',
    'ทับสะแก',
    'บ้านกรูด',
    'บางสะพานใหญ่',
    'บางสะพานน้อย',
    'มาบอำมฤต',
    'ปะทิว',
    'ชุมพร',
    'สวี',
    'หลังสวน',
    'ละแม',
    'ท่าชนะ',
    'ไชยา',
    'คีรีรัฐนิคม',
    'สุราษฎร์ธานี',
    'นาสาร',
    'บ้านส้อง',
    'ฉวาง',
    'คลองจันดี',
    'นาบอน',
    'ชุมทางทุ่งสง',
    'ตรัง',
    'กันตัง',
    'ชุมทางเขาชุมทอง',
    'ชะอวด',
    'พัทลุง',
    'ชุมทางหาดใหญ่',
    'ปาดังเบซาร์',
    'ปัตตานี',
    'ยะลา',
    'ตันหยงมัส',
    'สุไหงโก-ลก'
  ],
  [
    'วงเวียนใหญ่',
    'ตลาดพลู',
    'คลองต้นไทร',
    'จอมทอง',
    'วัดไทร',
    'วัดสิงห์',
    'การเคหะ',
    'บางบอน',
    'รางสะแก',
    'รางโพธิ์',
    'สามแยก',
    'พรมแดน',
    'ทุ่งสีทอง',
    'บางน้ำจืด',
    'คอกควาย',
    'บ้านขอม',
    'คลองจาก',
    'มหาชัย'
  ],
  [
    'บ้านแหลม',
    'ท่าฉลอม',
    'บ้านชีผ้าขาว',
    'คลองนกเล็ก',
    'บางสีคต',
    'บางกระเจ้า',
    'บ้านบ่อ',
    'บางโทรัด',
    'บ้านกาหลง',
    'บ้านนาขวาง',
    'บ้านนาโคก',
    'เขตเมือง',
    'ลาดใหญ่',
    'บางกระบูน',
    'แม่กลอง'
  ]
];
